...
def get_required_kw_args(fn): #获取无默认值命名关键字参数
    '''
...
def has_named_kw_args(fn): #判断是否存在命名关键字参数
    params = inspect.signature(fn).parameters
    for name,param in params.items():
        if param.kind == inspect.Parameter.KEYWORD_ONLY:
            return True

def has_var_kw_arg(fn):  #判断是否存在关键字参数
    params = inspect.signature(fn).parameters
    for name,param in params.items():
        if param.kind == inspect.Parameter.VAR_KEYWORD:
            return True

def has_request_arg(fn):  #判断是否有'request'的参数,且位置在最后
...
class RequestHandler(object):
    ...
    async def __call__(self,request):
        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args: # 若视图函数有命名关键词或关键词参数
            if request.method == 'POST':
                ...
                if not request.content_type:
                    return web.HTTPBadRequest('Missing Content-Type.') # 如果content_type不存在，返回400错误
        ...
        if kw is None: # 若request中无参数
            ...
            #check named arg: # 将request.match_info中的参数传入kw
...
def add_routes(app,module_name):
    ...
    for attr in dir(mod):
        if attr.startswith('_'): # 忽略'_'开头的对象，直接继续for循环
            continue
        fn = getattr(mod,attr)
        # 确保是函数
        if callable(fn):
