...
async def data_factory(app,handler):
    async def parse_data(request):
        if request.method == 'POSt':
            ...
            elif request.content_type.startswith('application/x-www-for-urlencoded'):
    ...
    async def response(request):
        ...
        if isinstance(r,dict):
            ...
            if template is None: # 不带模板信息，返回json对象
                ...
                # 调用Template对象的render()方法，传入r渲染模板，返回unicode格式字符串，将其用utf-8编码s
...
async def init(loop):
    ...
    add_static(app)
    srv = await loop.create_server(app.make_handler(),host=configs.server.host,port=configs.server.port)
    logging.info('server started at http://%s:%d' % tuple(configs.server.values()))
    return srv
